...
class Linear(Modulax):
    def __init__(self, in_features: jnp.ndarray, out_features: jnp.ndarray, parameters: dict = None) -> None:
...
def evaluate_cartpole(genome: jnp.ndarray, layer_dimensions: list, rng_eval, save: bool = False):
    # SECTION genome to model
    model_parameters = genome_to_param(genome, d=1, layer_dimensions=layer_dimensions)
    model = Sequential(
        Linear(in_features=layer_dimensions[0], out_features=layer_dimensions[1], parameters=model_parameters[0]),
        ReLU(),
        Linear(in_features=layer_dimensions[1], out_features=layer_dimensions[2], parameters=model_parameters[1]),
        Sigmoid())
    # !SECTION
    rng = rng_eval
    ...
    if save:
        ...
        vis.animate(f"anim/{int(time.time())}__cartpole_r_{cum_rewards[-1]}.gif", view=False)
...
def run():
    ...
    es_logging = ESLog(num_dims=N_NEURONS, num_generations=MAX_GEN, top_k=3, maximize=True)
    ...
    for generation in tqdm(range(MAX_GEN), desc='Generation loop', position=0):
        ...
        if (generation+1) % 5 == 0:
            print(f'[INFO - OpenES]: Gen: {generation+1} | Fitness {log["log_top_1"][generation]}') # state.best_fitness:.5f}

    print(state.best_fitness)
    fig, ax = es_logging.plot(log, "CartPole Env, Gene encoding", )  # ylims=(0, 30)
