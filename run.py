from gene.evaluatax import evaluate_model_vmap, _evaluate_small_model
...
def run_expe(algorithm: str = 'SNES', pop_size: int = 10, max_gen: int = 3):
    logger = logging.getLogger("logger")
    ...
    for generation in tqdm(range(max_gen), desc='Generation loop', position=0):
        ...
        rng, rng_gen, rng_eval = jax.random.split(rng, 3)
        # NOTE: Ask generation | x (popsize, num_dims)
        x, state = strategy.ask(rng_gen, state, es_params)
        # NOTE: Evaluate generation[s] genome
        # temp_fitness = evaluate_model_vmap(x)
        temp_fitness = jnp.array([_evaluate_small_model(genome) for genome in x])
        # after fitness evaluation, apply fitness reshaper: https://github.com/RobertTLange/evosax/blob/main/examples/00_getting_started.ipynb
        fitness = fit_shaper.apply(x, temp_fitness)
        
        # NOTE: Tell / Update step
        state = strategy.tell(x, fitness, state, es_params)
...
if __name__ == "__main__":
    ...
    if not logpath.exists():
        logpath.mkdir()
    # NOTE: add HP to logging info (extend class and stuff)
    logger = logging.getLogger("logger")
    ...
    # with jax.profiler.trace('/profiles/jax-trace', create_perfetto_link=True, create_perfetto_trace=True):
    run_expe()