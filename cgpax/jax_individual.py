...
def compute_cgp_genome_mask(config: dict, n_in: int, n_out: int) -> jnp.ndarray:
    ...
    if config.get("recursive", False):
        ...
        in_mask = jnp.minimum(
            config["levels_back"] * jnp.ones(n_nodes), jnp.arange(n_in, n_in + n_nodes)
        )
    else:
...
def generate_population(
    ...
    partial_generate_genome = partial(
        generate_genome,
...
def lgp_one_point_crossover_genomes(
    genome1: jnp.ndarray, genome2: jnp.ndarray, rnd_key: random.PRNGKey
...
def one_point_crossover_genomes(
    genome1: jnp.ndarray, genome2: jnp.ndarray, rnd_key: random.PRNGKey
) -> Tuple[jnp.ndarray, jnp.ndarray]:
    assert len(genome1) == len(genome2)
    rnd_key, xover_key = random.split(rnd_key, 2)
...
def mutate_genome(
    ...
    new_genome = generate_genome(
        genome_mask, new_genome_key, genome_transformation_function
    )
...
def mutate_genome_n_times_stacked(
    genome: jnp.ndarray,
    rnd_key: random.PRNGKey,
    n_mutations: int,
    genome_mask: jnp.ndarray,
    ...
    mutated_genomes, _, _, _, _ = fori_loop(
        0, n_mutations, mutate_and_store, (genomes, genome, rnd_key)
    )
    return mutated_genomes


def mutate_genome_n_times(
    genome: jnp.ndarray,
    rnd_key: random.PRNGKey,
    n_mutations: int,
    genome_mask: jnp.ndarray,
