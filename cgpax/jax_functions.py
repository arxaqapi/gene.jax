...
class JaxFunction:
    def __init__(
        self, op: Callable[[float, float], float], arity: int, symbol: str = None
    ) -> None:
...
available_functions = {
    ...
    "prot_div": JaxFunction(
        lambda x, y: cond(y == 0, jit(lambda a, b: 1.0), jnp.divide, x, y), 2, "/"
    ),
    "abs": JaxFunction(lambda x, y: jnp.abs(x), 1, "|.|"),
    "exp": JaxFunction(lambda x, y: jnp.tanh(jnp.exp(x)), 1, "exp"),
