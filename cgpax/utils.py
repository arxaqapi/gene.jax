...
def compute_active_size(genome: jnp.ndarray, config: dict) -> Tuple[int, int]:
    if config["solver"] == "cgp":
        ...
        x_genes, y_genes, f_genes, out_genes = jnp.split(
            genome, jnp.asarray([n_nodes, 2 * n_nodes, 3 * n_nodes])
        )
...
def compute_active_graph(
    x_genes: jnp.ndarray,
    y_genes: jnp.ndarray,
    f_genes: jnp.ndarray,
    out_genes: jnp.ndarray,
    config: dict,
) -> np.ndarray:
    n_nodes = config["n_nodes"]
    n_in = config["n_in"]
...
def __compute_active_graph__(
    ...
    if not active[idx]:
        active[idx] = True
        __compute_active_graph__(
            active, x_genes, y_genes, f_genes, n_in, int(x_genes[idx - n_in])
        )
        arity = list(available_functions.values())[f_genes[idx - n_in]].arity
        if arity > 1:
            __compute_active_graph__(
                active, x_genes, y_genes, f_genes, n_in, int(y_genes[idx - n_in])
            )
...
def readable_cgp_program_from_genome(genome: jnp.ndarray, config: dict) -> str:
    ...
    for buffer_idx in range(n_in, len(active)):
        if active[buffer_idx]:
            ...
            text_function += (
                f"  buffer[{buffer_idx}] = {function.symbol}(buffer[{x_genes[idx]}]"
            )
            if function.arity > 1:
...
def compute_coding_lines(
    ...
    output_registers = set(
        range(config["n_registers"] - config["n_out"], config["n_registers"])
    )
    for row_id in range(config["n_rows"] - 1, -1, -1):
        if int(lhs_genes[row_id]) in output_registers:
            ...
            __compute_coding_lines__(
                active, lhs_genes, x_genes, y_genes, f_genes, row_id
            )
    return active


def __compute_coding_lines__(
    active: np.ndarray,
    lhs_genes: jnp.ndarray,
    x_genes: jnp.ndarray,
    y_genes: jnp.ndarray,
    ...
    for new_row_id in range(row_id - 1, -1, -1):
        if lhs_genes.at[new_row_id].get() == x_genes.at[row_id].get():
            __compute_coding_lines__(
                active, lhs_genes, x_genes, y_genes, f_genes, new_row_id
            )
            break
...
def readable_lgp_program_from_genome(genome: jnp.ndarray, config: dict) -> str:
    ...
    for row_idx in range(config["n_rows"]):
        if active[row_idx]:
            ...
            text_function += (
                f"  r[{lhs_genes[row_idx]}] = {function.symbol}(r[{x_genes[row_idx]}]"
            )
            if function.arity > 1:
...
def lgp_graph_from_genome(
    ...
    for row_id in range(n_rows - 1, -1, -1):
        if (
            lhs_ids[row_id] is not None
            or int(lhs_genes.at[row_id].get()) in missing_outputs
        ):
            ...
            x_ids[row_id] = x_name
            lhs_ids, x_ids, y_ids = __reassign_variables__(
                lhs_genes,
                x_genes,
                y_genes,
                lhs_ids,
                ...
                y_ids,
                row_id,
                x_genes.at[row_id].get(),
                x_name,
            )
            if function.arity > 1:
                y_register = y_genes.at[row_id].get()
                y_name = (
                    f"h_{row_id}" if y_register > config["n_in"] else f"i_{y_register}"
                )
                y_ids[row_id] = y_name
                lhs_ids, x_ids, y_ids = __reassign_variables__(
                    lhs_genes,
    ...
    for row_id in range(n_rows):
        if lhs_ids[row_id] is not None:
            ...
            x = (
                x_ids[row_id]
                if x_ids[row_id].startswith("i_")
                else renaming_dict.get(x_ids[row_id], "0")
            )
            graph.add_edge(x, lhs, color=x_color)
            if y_ids[row_id] is not None:
                y = (
                    y_ids[row_id]
                    if y_ids[row_id].startswith("i_")
                    else renaming_dict.get(y_ids[row_id], "0")
                )
