...
def __update_rewards_carry__(
    rew_carry: Tuple[int, int, int], state: State, active_episode: int
) -> Tuple[int, int, int]:
    rew_forward, rew_ctrl, rew_healthy = rew_carry
    rew_forward += (
        state.metrics.get("reward_forward", state.metrics.get("reward_run", 0))
        * active_episode
    )
    rew_ctrl += state.metrics.get("reward_ctrl", 0) * active_episode
    rew_healthy += (
        state.metrics.get("reward_healthy", state.metrics.get("reward_survive", 0))
        * active_episode
    )
...
def __init_detailed_rewards_carry__(
    episode_length: int,
) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, int]:
    return (
        jnp.zeros(episode_length),
        jnp.zeros(episode_length),
        jnp.zeros(episode_length),
        jnp.zeros(episode_length),
        0,
...
def __update_detailed_rewards_carry__(
    rew_carry: Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, int],
    state: State,
    active_episode: int,
) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, int]:
...
def __evaluate_program_global__(
    ...
    (final_env_state, _, cum_reward, reward_carry, _), _ = lax.scan(
        ...
        init=(
            initial_env_state,
            jnp.zeros(program_state_size),
            initial_env_state.reward,
    ...
    x_distance = final_env_state.metrics.get(
        "x_position", 0
    ) - initial_env_state.metrics.get("x_position", 0)
...
def __evaluate_program__(
    ...
    return __evaluate_program_global__(
        program,
        program_state_size,
        rnd_key,
        env,
        episode_length,
        __init_rewards_carry__,
        __update_rewards_carry__,
        __extract_final_rewards_carry__,
    )


def __evaluate_program_detailed_tracking__(
    program: Callable,
    program_state_size: int,
    rnd_key: random.PRNGKey,
    env: EpisodeWrapper,
    ...
    return __evaluate_program_global__(
        ...
        env,
        episode_length,
        __init_detailed_rewards_carry__,
        __update_detailed_rewards_carry__,
        __extract_final_detailed_rewards_carry__,
    )


def __evaluate_genome_n_times__(
...
def evaluate_cgp_genome_n_times(
    genome: jnp.ndarray,
    rnd_key: random.PRNGKey,
    config: dict,
    env: EpisodeWrapper,
    ...
    return __evaluate_genome_n_times__(
        evaluate_cgp_genome,
        genome,
        rnd_key,
        config,
...
def evaluate_lgp_genome_n_times(
    genome: jnp.ndarray,
    rnd_key: random.PRNGKey,
    config: dict,
    env: EpisodeWrapper,
...
def evaluate_cgp_genome(
    ...
    """Turn the genome into a program and calls `inner_evaluator(...)` on it."""
    return inner_evaluator(
        genome_to_cgp_program(genome, config),
        config["buffer_size"],
        rnd_key,
        env,
        episode_length,
...
def evaluate_lgp_genome(
    ...
    return inner_evaluator(
        genome_to_lgp_program(genome, config),
        config["n_registers"],
        rnd_key,
        env,
        episode_length,
...
def evaluate_cgp_genome_regression(
    genome: jnp.ndarray, config: dict, observations: jnp.ndarray, targets: jnp.ndarray
) -> float:
    def predict_row(
        observation: jnp.ndarray, genome: jnp.ndarray, config: dict
    ) -> jnp.ndarray:
