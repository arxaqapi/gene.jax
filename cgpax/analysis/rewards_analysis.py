...
from cgpax.jax_evaluation import (
    evaluate_cgp_genome,
    evaluate_lgp_genome,
    __evaluate_program_detailed_tracking__,
)
from cgpax.run_utils import (
    __init_environment_from_config__,
    __update_config_with_env_data__,
)


def compute_rewards_df(
    genome: jnp.ndarray, rnd_key: random.PRNGKey, config: dict
) -> pd.DataFrame:
    ...
    if config["solver"] == "cgp":
        result = evaluate_cgp_genome(
            genome, rnd_key, config, env, inner_evaluator=inner_evaluator
        )
    elif config["solver"] == "lgp":
        result = evaluate_lgp_genome(
            genome, rnd_key, config, env, inner_evaluator=inner_evaluator
        )
    else:
        raise ValueError
    ...
    df = pd.DataFrame(
        list(zip(healthy, ctrl, forward, total)),
        columns=["healthy", "ctrl", "forward", "total"],
    )
    ...
    for outer_seed in range(max_seed):
        for folder in os.listdir("genomes"):
            ...
            for inner_seed in range(max_seed):
                ...
                target_f = (
                    f"{folder.replace(f'_{outer_seed}', f'_{inner_seed}')}_{gen}.csv"
                )
                if target_f not in os.listdir("data/rewards"):
                    write_rewards_df(
                        base_p, outer_seed, gen, f"data/rewards/{target_f}", rand_key
                    )
